Embedding error: Unable to create tensor, you should probably activate truncation and/or padding with 'padding=True' 'truncation=True' to have batched tensors with the same length. Perhaps your features (input_ids in this case) have excessive nesting (inputs type list where type int is expected).
...
class SafeEmbeddingModel:
    ...
    def encode_safe(self, documents, batch_size=32, show_progress=True):
        ...
        try:
            ...
            if "too large" in str(e) or "out of memory" in str(e):
            ...
            elif "padding" in str(e) or "truncation" in str(e):
    ...
    with st.sidebar:
        ...
        if uploaded_file is not None:
            ...
            if st.button("🚀 Compute Embeddings & Enable Interactive Mode", type="primary"):
                ...
                # Step 2: Load model and compute embeddings
